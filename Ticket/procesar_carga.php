...
$id_area_usuario = $_SESSION['id_area_usuario']; // Usar el ID de área de usuario de la sesión
$id_prioridad_ticket = $_POST['id_prioridad_ticket'];
$id_estado_ticket = $_POST['id_estado_ticket']; // Se asume que el estado se establece durante la carga

// Verificar si se ingresó una ubicación
$latitud = $_POST['ubicacion_lat'];
$longitud = $_POST['ubicacion_lng'];
$direccion = $_POST['search-input']; // El campo de búsqueda de la dirección

try {
    // Iniciar la transacción
    $DB_conn->begin_transaction();

    // Insertar la ubicación si se seleccionó
    if (!empty($latitud) && !empty($longitud)) {
        ...
        $id_ubicacion = $DB_conn->insert_id; // Obtiene el último ID insertado
    } else {
    ...
    $query_insert_ticket = "INSERT INTO tickets (titulo_ticket, descripcion_ticket, id_usuario_ticket, id_area_ticket, id_prioridad_ticket, id_estado_ticket, id_ubicacion) 
                            VALUES ('$titulo_ticket', '$descripcion_ticket', $id_usuario_ticket, $id_area_usuario, $id_prioridad_ticket, $id_estado_ticket, $id_ubicacion)";
